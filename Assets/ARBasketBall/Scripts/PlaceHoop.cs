        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                if (m_RaycastManager.Raycast(touch.position, s_Hits, TrackableType.PlaneWithinPolygon))
                {
                    ...
                    Vector3 ballSpawnPosition = m_ARCamera.transform.position + new Vector3(0f,-0.5f,1.5f); 
                    spawnedBall = Instantiate(m_BallPrefab, ballSpawnPosition, Quaternion.identity);
                    spawnedBall.transform.parent = m_ARCamera.transform;
                    spawnedBall.GetComponent<ballControlScript>().enabled = true;
